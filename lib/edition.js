...
function edition(fields) {
  ...
  addFields(['title', 'summary'], obj, fields)
  ...
  function publishEdition(path) {
    var writeStream = fs.createWriteStream(path)
      , urlStr = fields.url ? fields.url + '/' + basename(path) : path
      , relativeUrl = fields.url ? url.parse(fields.url).pathname + '/' + basename(path) : path
      , contents =
        { _attr:
          { xmlns: 'http://www.w3.org/2005/Atom'
          , 'xmlns:app': 'http://www.w3.org/2007/app'
          }
        , entry: _.map(obj.pages, function (p) { return p.object })
        , link:
          { _attr:
            { rel: 'self'
            , href: urlStr
  ...
  function publishPage(ee, path, page) {
    var writeStream = page.publish(path + '-' + page.object.id)
